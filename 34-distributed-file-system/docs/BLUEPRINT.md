...
replica placement under rack constraints, liveness via heartbeats, durability
via checkpoints, and the safe-mode bootstrap. It documents the system **as
...
detection, under-replication scheduling, safe mode, and checkpoint persistence.
Out of scope: NameNode HA, an edit log/WAL, pipelined writes, erasure coding,
authentication, encryption, and on-the-wire binary block frames. These are
enumerated honestly rather than implied.
...
flowchart TD
    ...
    Snapshot[("checkpoint.json (manual save and load)")]
...
is no edit log / write-ahead log. `_directories` is pre-seeded with the root
`"/"` so that the namespace is never empty.
...
- **Liveness and recovery**: that a node silent past 30 s is evicted, its
  blocks removed from the location map, and under-replication scheduled.
...
`pytest-asyncio` runs in `asyncio_mode = "auto"`. The whole suite runs
in-process with no live NameNode/DataNode sockets required.
...
2. **No NameNode HA.** Single process, single point of failure. There is no
   Secondary NameNode, no JournalNodes, no Active/Standby pair.
...
4. **`replicate` command is a stub.** When the NameNode tells a DataNode to
   replicate a block elsewhere, the DataNode logs the intent but does not copy
   the bytes. Re-replication after DN loss is *scheduled* but does not run.
   This is the most consequential gap for self-healing claims.
...
6. **No authentication, no encryption, no quotas-on-the-wire.** It is a
   single-tenant testbed. `owner`/`group`/`permission` exist but aren't
   enforced.
...
8. **No append at the block level.** `append(path, data)` reads the whole
   existing file, concatenates, deletes, and rewrites.
9. **Erasure coding is enum-only.** `ReplicationPolicy.ERASURE_CODING` exists;
   nothing produces or decodes EC stripes.
10. **`checkpoint_interval` is configured but not driven.** Nothing triggers
    periodic checkpoints; an operator must call `save_checkpoint`.
11. **Safe-mode reporting only counts blocks the NameNode already knows
    about.** A fresh NameNode with no `_blocks` exits safe mode immediately.
12. **`rename` of a large subtree is O(N).** Every descendant key in `_files`
    and `_directories` is rewritten.
13. **Pending-replication consumption is best-effort.** A
    `_pending_replications` entry is dispatched only when a DN that holds the
    block heartbeats; if no holder heartbeats before a restart, it is lost.
