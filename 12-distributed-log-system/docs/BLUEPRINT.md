...
The concepts this codebase teaches are: log-structured storage with sparse indexing, the leader/follower replication and ISR model, watermark-based read visibility, controller-driven failover and leader election, consumer-group rebalancing, and idempotent/exactly-once delivery. The scope is a faithful in-process implementation of these mechanics with a gRPC transport layer; it is a teaching and exploration codebase rather than a hardened production broker. Compaction-candidate scanning, key-based partitioning against the real partition count, on-disk segment deletion, and multi-topic follower fetch are all implemented; the one remaining simplification is that compaction rewrites survivors into a working directory and reports the result without atomically swapping the compacted segments back in place, as the "What's Real vs Simulated" section of the README states.
...
Partition selection (`partition_for`): if a record has a key, the partition is `crc32(key) % num_partitions`, where `num_partitions` is the target topic's actual partition count (registered via `Producer::set_partition_count`, falling back to `DEFAULT_PARTITION_COUNT` when unknown), so the same key always lands in the same partition (preserving per-key ordering) and keyed traffic spreads across every partition the topic actually has; without a key, an atomic counter round-robins modulo the same real count. Key-based partitioning is the mechanism behind per-key ordering guarantees: because all records for a key route to one partition, and a partition is a totally ordered log, a consumer sees a key's records in produce order.
...
There are two distinct cleanup policies, captured by `CleanupPolicy`. *Delete* retention discards whole old segments; *compact* retention keeps the latest value per key. `RetentionManager` and `LogCleaner` implement deletion: a segment is eligible if it is older than `retention_ms`, and beyond that the oldest segments are deleted until total size falls under `retention_bytes`. Because retention operates at segment granularity, reclaiming space is a file delete, not a rewrite of live data — which is the whole reason the log is segmented in the first place. Deletion is real reclamation: `Partition::delete_segment` drops the segment from the in-memory map *and* unlinks its `.log`, `.index`, and `.timeindex` files from disk (tolerating already-missing files), so the bytes are actually freed.
...
3. Swap the rewritten segments in for the originals (see the note below on the current limitation here).

Records without a key are always kept, since compaction is meaningless without a key to dedupe on. Candidate discovery is wired up: `Partition::segment_metadata` exposes the real per-segment metadata (base offset, size, last-modified time, max timestamp), `LogCleaner::get_segment_metadata` reads it so `cleanable_ratio` and retention see actual segments, and `LogCleaner::compaction_candidates` turns the partition's non-active segments into the `SegmentInfo` list the compactor consumes. Compaction then finds those candidates and reduces each key to its latest value (verified by `test_compaction_reduces_to_latest_value_per_key`, which collapses six records over three keys down to three). The remaining limitation is the final step: the compactor writes survivors into a working directory and reports the results, but does not yet atomically rename them back over the originals in place.
...
- **Compaction in-place swap.** Candidate discovery and the dedup rewrite are complete — `LogCompactor` finds candidate segments via the partition's real segment metadata and reduces each key to its latest value — but the compactor writes survivors into a working directory and reports the result rather than atomically renaming the compacted segments back over the originals. Wiring that final swap (rename + delete originals + update partition metadata) is the remaining step.
...
`CleanupPolicy` selects between the two reclamation models; `SegmentMetadata` is what `cleanable_ratio` consumes to decide whether a partition is worth compacting (populated by `get_segment_metadata` from the partition's real segment set); `CleanResult` and `CleanerStats` report what a cleaning pass accomplished.
...
Correctness is verified by 181 integration test functions across five files plus 47 inline unit tests (228 total), with no external services — every test uses `tempfile` for storage so runs are hermetic.
